...
public class MazeMaker : MazeAlgorithm {
	...
	public MazeMaker(MazeCell[,] mazeCells) : base(mazeCells) {
	}
	...
	private void Kill() {
		while (RouteStillAvailable (currentRow, currentColumn)) {
			int direction = Random.Range (1, 5); //Generates a random path for the generator to follow.
		...
		for (int r = 0; r < mazeRows; r++) {
			for (int c = 0; c < mazeColumns; c++) {
				if (!mazeCells [r, c].visited && CellHasAnAdjacentVisitedCell(r,c)) {
					courseComplete = false; //if it found a cell that doesnt have one or more of its walls broken, then the process repeats for that cell.
					currentRow = r;
					currentColumn = c;
